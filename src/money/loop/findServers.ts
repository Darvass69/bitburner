...
// get every server and gain root access when possible. add the result to a port
...
export async function main(ns: NS): Promise<void> {
	...
	ns.writePort(config.ports.serverList, JSON.stringify(serverList))
}
	...
	if (isWeak(ns, server)) {
		if (ns.hasRootAccess(server)) {
			...
			if (ns.getServerMaxRam(server) == 0){
				...
				ns.print(`gained root: ${server}`)
				serverList.root.push(server)
			}
			else {
				//! print error
				ns.print(`ERROR: failed to gained root access on ${server}`)
				serverList.noRoot.push(server)
			}
		}
	}
